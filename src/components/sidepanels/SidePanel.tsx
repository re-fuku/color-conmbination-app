...
export default function SidePanel(props: SidePanelProps) {
    const hoverStyle = "hover:border-white hover:border-2" // hover時のスタイル

    return (
        <div className="w-75 h-full bg-panel-bg-color flex flex-col border-none">
            <Group orientation="vertical" className="h-full">
                <Panel className="scrollbar-hide rounded-bl-2xl bg-panel-bg-color" defaultSize={50} minSize={20} collapsible={true}>
                    <SettingPanel 
                        {...props}
                    />
                </Panel>
